...
const getIdentity = () => {
  try {
    const stored = localStorage.getItem("peaknft5_key");
    if (stored) return Ed25519KeyIdentity.fromJSON(stored);
  } catch (e) {}
  const id = Ed25519KeyIdentity.generate();
  localStorage.setItem("peaknft5_key", JSON.stringify(id.toJSON()));
  return id;
};
...
export const connectToIC = async () => {
  ...
  if (!CANISTER_ID) throw new Error("No canister ID. Run: dfx deploy && node setup-env.js");
